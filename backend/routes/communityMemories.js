...
router.get('/', readLimit, async (req, res) => {
  ...
  const [memories, feature] = await Promise.all([
    ...
    .limit(12)
    .lean(),
    CommunityMemoryFeature.findOne({ dateKey: getVietnamDateKey(), expiresAt: { $gt: new Date() } }).select('images').lean(),
  ])
  res.set('Cache-Control', 'public, s-maxage=20, stale-while-revalidate=40')
  res.json({ memories: memories.map(serialize), featuredImages: feature?.images || [], dateLabel: formatVietnamDate() })
})
...
router.put('/featured/:slot', publishLimit, async (req, res) => {
  ...
  const existing = await CommunityMemoryFeature.findOne({ dateKey: getVietnamDateKey() }).lean()
  ...
  const feature = await CommunityMemoryFeature.findOneAndUpdate(
    ...
    { $set: { images, updatedBy: user._id, expiresAt: end } },
